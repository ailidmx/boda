# Preguntas Puzzle Prioritarias / Questions puzzle prioritaires

Estado: open | decided | blocked
Prioridad: P1 | P2 | P3

| ID | Prioridad | Estado | Tema | Pregunta | Owner | Tarea ligada |
|---|---|---|---|---|---|---|
| Q-001 | P2 | open | Vestuario novios | Que elementos compartidos concretaran la estetica oaxaquena coordinada para ambos? | Ayde + David | T-036 |
| Q-002 | P2 | blocked | Vestuario monsieur | Cuales 3 lineas de accesorios para monsieur probamos primero? | Ayde + David | T-038 |
| Q-003 | P1 | open | Proveedor taquiza | Cual es el contacto final y condiciones de taquiza del sabado noche? | Manu | T-025 |
| Q-004 | P1 | open | Tejuino | Quien provee tejuino, en que horario y con que volumen? | David + Manu | T-026 |
| Q-005 | P1 | open | Musica | Como distribuimos exactamente los bloques Mariachi, Marimba, Norteno y Karaoke por hora? | David + Ayde | T-031 |
| Q-006 | P1 | open | Spotify API | Que listas Spotify ya existen y cual sera el flujo de integracion en la interfaz? | David + Copilot | T-033 |
| Q-007 | P1 | open | Costos invitados | Cual sera el texto final FR/ES/EN para explicar costos de estancia sin friccion? | David + Ayde | T-029 |
| Q-008 | P1 | open | Cabanas | Que 10 plazas libres reservamos para extranjeros o cambios de ultima hora? | David + Ayde | T-008 |
| Q-009 | P2 | open | Beneficios club | Cual es la lista final validada con venue de beneficios incluidos? | David + Copilot | T-030 |
| Q-010 | P2 | open | Bagues | Que detalles finales faltan registrar sobre bagues para cerrar ficha? | David + Ayde | T-040 |
| Q-011 | P2 | open | Traje de Cuilapam | Cual es el nombre del artesano o taller y que tecnica, materiales y autoria tiene cada bordado dorado? | Ayde + David | T-037 |
| Q-012 | P1 | open | Viaje Dimitar | Cuales son los cuatro vuelos exactos del billete de Dimitar, con numeros y horarios locales? | David + Dimitar | T-055 |
| Q-013 | P1 | open | Estancia Dimitar | Donde se hospedara del 14 al 19 y del 21 al 28, y Dimitar +1 comparte vuelos y estancias? | David + Dimitar | T-056 |
| Q-014 | P1 | open | Viaje Mama Tina | Se confirman las fechas 10 de febrero a 15 de marzo y cuales seran sus vuelos, estancias y traslados? | Ayde + Mama Tina | T-059 |

## Proximo sprint de 5 preguntas (recomendado)
1. Q-003
2. Q-004
3. Q-005
4. Q-006
5. Q-007
