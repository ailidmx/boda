...
The repo uses a **single-branch flow**: everything goes straight to `master`.
There is NO `develop` branch and NO feature branches. `master` is the default
branch, has NO branch protection, and is what deploys to production. Commit
directly to `master` and push.

### Workflow
1. Work directly on `master`: `git checkout master && git pull`.
2. Commit your changes with a clear message (conventional commits, e.g.
   `feat(...)`, `fix(...)`, `docs(...)`).
3. Push straight to `master`: `git push origin master`. CI runs on the push and
   deploys on the merged branch.
4. Keep the working tree clean — commit or discard scratch/inspection scripts
   (e.g. `scripts/_inspect_*.mjs`) rather than leaving them untracked.

### Notes
- The GitHub MCP server (`github.com/github/github-mcp-server`) is available for
  repo operations. Repo is `ailidmx/boda`.
- Deploy checks (`deploy-production`, `deploy-functions`) run on the merged
  branch — a green push does not mean a deploy happened yet.
- The only other branch kept around is `refactor/architecture` (a large,
